...
pass_en = tk.Entry(window, fg="black", bg="white", width = 50, show="*")
pass_en.pack(anchor='w', padx=20)
...
def cbfx():
    ...
    passwd = pass_en.get()
    pass_hash = sh.sha256(passwd.encode('utf-8')).hexdigest()
    ...
    p_subject_list = ', '.join(subject_list)

    db.insert_data(email_en.get(), pass_hash,  ch_branch(), p_subject_list)
